...
RUN apt-get update \
 ...
 && curl -Lo vault.zip https://releases.hashicorp.com/vault/1.5.3/vault_1.5.3_linux_amd64.zip \
     && unzip vault.zip \
     && mv vault /usr/bin/vault \
     && chmod 0755 /usr/bin/vault \
     && rm vault.zip \
 && curl -Lo /usr/bin/safe https://github.com/starkandwayne/safe/releases/download/v1.5.7/safe-linux-amd64 \
 ...
 && curl -Lo /usr/bin/bosh https://github.com/cloudfoundry/bosh-cli/releases/download/v6.4.0/bosh-cli-6.4.0-linux-amd64 \
     && chmod 0755 /usr/bin/bosh

RUN curl -Lo credhub_cli.tgz https://github.com/cloudfoundry-incubator/credhub-cli/releases/download/2.8.0/credhub-linux-2.8.0.tgz \
